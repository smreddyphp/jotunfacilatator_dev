...
.assign_a,.report_a{
                        ...
                        <div class="panel-body pn">
							...
							<thead>
								<tr role="row">
								    ...
								    <th class="sorting_asc" tabindex="0" aria-controls="example3" rowspan="1" colspan="1" aria-sort="ascending" aria-label="Sr.No: activate to sort column descending" style="width: 46px;">Image</th></th>
								    ...
								    <th class="sorting" tabindex="0" aria-controls="example3" rowspan="1" colspan="1" aria-label="Exam Open Date Time: activate to sort column ascending" style="width: 174px;">Registered</th>
								    <th class="sorting" tabindex="0" aria-controls="example3" rowspan="1" colspan="1" aria-label="Exam Close Time: activate to sort column ascending" style="width: 136px;">User</th>
								    ...
								    <th class="sorting" tabindex="0" aria-controls="example3" rowspan="1" colspan="1" aria-label="Exam Duration In Minutes: activate to sort column ascending" style="width: 202px;">View</th>
								    <!-- <th class="sorting" tabindex="0" aria-controls="example3" rowspan="1" colspan="1" aria-label="Exam Duration In Minutes: activate to sort column ascending" style="width: 202px;">Assign Task</th> -->
											...
											<tbody>
											    ...
											    if(!empty($users))
											    {
											        ...
											        <tr class="odd" role="row">
                                        				...
                                        				<span class="label label-success" onclick="changestatusteam(<?php echo $user->id ?>,'1')">Active</span>
                                        				<?php }else{?>
                                        				<span class="label label-danger" onclick="changestatusteam(<?php echo $user->id ?>,'0')">Inactive</span>
                                        				<?php } ?>
                                                        </td>
                                                        <td><a href="<?php echo base_url();?>users/edit/<?php echo $user->id; ?>" class="btn btn-xs btn-success"><span >Edit</span></a></td>
                                                          <?php if($this->lib_auth->getRoleID() == 1){ ?>
                                                        	<td><span class="label label-danger" onclick="deletedata(<?php echo $user->id; ?>)">Delete</span></td>  
                                                        <?php } ?>
    											    </tr>-->
    											    <?php if($user->profile_image == ""){?>
    											    <td><img src="<?php echo base_url(); ?>server/assets/media/noimage.png"  height="50px" width="50px"  class="media-object img-circle"></td>
                                    				<?php }else{?>
                                                    <td><img src="<?php echo base_url(); ?>server/assets/media/<?php echo $user->profile_image; ?>" height="50px" width="50px" class="media-object img-circle"> <?php //echo img(array('src' => $this->config->item('base_url').$this->config->item('shops_image_path').$user->profile_image), '','class="media-object img-circle", style=""'); ?></td>
                                    				<?php } ?>
                                                    <td><?php echo $user->code; ?></td>
                                                    <td><?php echo $user->email; ?></td>
                                                    ...
                                                    <td>
                                                        <?php
                                                        echo $this->db->get_where("roles",array('id'=>$user->role_id))->row()->code;
                                                        ?>
                                                    </td>
                                                    <td>
                                                      <span class="label <?php echo ($user->status == 1) ? 'label-success' : 'label-danger'; ?>">
                                                          <?php echo $this->config->item('users_status')[$user->status]; ?>
                                                      </span>
                                                    </td>
                                                    <td><a href="<?php echo base_url(); ?>sales/forms/<?php echo $user->id ?>" class="btn btn-success br2 btn-xs fs12">View Works</a><?php // echo anchor(base_url($this->config->item('sales_view_uri').$user->id), 'View Forms', 'class="btn btn-xs btn-success"'); ?></td>
                                                    <!-- <td>
                                                       --<?php echo anchor(base_url($this->config->item('sales_ajax_add_task_uri').$user->id), 'Assign Task', 'class="btn btn-success br2 btn-xs fs12 dropdown-toggle add-task-link" data-popup="tooltip" title="Add"'); ?>-
                                                                                        
                                                       <a href="<?php echo base_url(); ?>sales/task/<?php echo $user->id ?>" class="btn btn-success br2 btn-xs fs12 assign_a">Assign Task</a>
                                                       <button onclick="report_view_forms('<?php echo $user->id ?>')" data-id="<?php echo $user->id ?>" class="btn btn-primary br2 btn-xs fs12"></button>
                                                        <button type="button" class="btn btn-primary waves-effect waves-light add_city" data-toggle="modal" data-target = "#add_city" data-id="<?php echo $user->id ?>" style="float: none;margin: 5px;">
                                                        Generate Report
                                                        </button>
                                                         <a href="<?php echo base_url(); ?>sales/report_forms/salesman/<?php echo $user->id ?>" class="btn btn-primary br2 btn-xs fs12 report_a">Generate Report</a>
                                                    </td> -->
											       <?php }
											    }
											    ?>
    ...
    <div class = "modal fade" id = "add_city" tabindex = "-1" role = "dialog" aria-labelledby = "myModalLabel" aria-hidden = "true">
    ...
    $(document).ready(function() {            
        ...
        $(document).on("click",'.add_city',function(event){ 
            ...
            $modal.load('<?php echo base_url();?>sales/reports_model',
            function(){            
              $modal.modal('show');
            });
        });
    
    /*function report_view_forms(id)
    {
        alert(id);
