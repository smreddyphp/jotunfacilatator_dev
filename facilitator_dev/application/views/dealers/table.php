<input type="hidden" value="dealers" id="table_name">
...
<?php } ?>
   ...
   <!-----------<td><?php echo $objUser->last_name; ?></td>---------->
				...
				<span class="label label-success" onclick="changestatusd(<?php echo $objUser->id ?>,'1')">Active</span>
				<?php }else{?>
				<span class="label label-danger" onclick="changestatusd(<?php echo $objUser->id ?>,'0')">Inactive</span>
				<?php } ?>
                    
                </td>
                <td><?php echo anchor(base_url($this->config->item('dealers_edit_uri').$objUser->id), 'Edit', 'class="btn btn-xs btn-success"'); ?></td>
  <?php if($this->lib_auth->getRoleID() == 1){ ?>
				<td class=""><span class="label label-danger" onclick="deletedealer(<?php echo $objUser->id; ?>)">Delete</span></td>  
<?php } ?>
            </tr>
        <?php endforeach; ?>
    <?php else: ?>
