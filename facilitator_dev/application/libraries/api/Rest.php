...
class REST
{
    ...
    function __construct($config = array())
    {
        ...
        // Config settings
        $this->rest_server = $this->_ci->config->item('rest_server_url');
        $this->http_auth = $this->_ci->config->item('http_auth');
        $this->http_user = $this->_ci->config->item('http_user');
        $this->http_pass = $this->_ci->config->item('http_pass');
        $this->api_name = $this->_ci->config->item('api_name');
        $this->api_key = $this->_ci->config->item('api_key');
        $this->ssl_verify_peer = $this->_ci->config->item('ssl_verify_peer');
        $this->ssl_cainfo = $this->_ci->config->item('ssl_cainfo');
    ...
    protected function _call($method, $uri, $params = array(), $format = NULL)
    {
        ...
        // IP Address
        //$this->http_header('P-IP-ADDRESS', $this->_ci->input->ip_address());

        // Set Token
        if($this->_ci->session->userdata('auth_token'))
            $this->http_header('P-Auth-Token', $this->_ci->session->userdata('auth_token'));

        // Set Domain
        $this->_ci->curl->option('REFERER', $this->_ci->config->item('SERVER_NAME'));
        ...
        $objResponse = $this->_format_response($response);
        ...
        if($this->status() === 401)
        {
            if($uri == 'logout/')
            {
                redirect('auth/logout/1', 'refresh');
            }
