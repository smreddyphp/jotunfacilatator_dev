...
abstract class REST_Controller extends \CI_Controller {
    ...
    /**
     * The server throws the CI errors
     */
    const ERR_REST_CI_EXCEPTION = 120001;
     ...
     */
    public function __construct($config = 'v1/api/rest')
    {
        parent::__construct();
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        if ($this->config->item('force_https') && $this->request->ssl === FALSE)
        {
            $this->response(
                ["response" => [
                    ...
                    "errors" => array(
                        array(
                            "code" => self::ERR_REST_UNSUPPORTED,
        ...
        if ($this->config->item('rest_enable_keys') && $use_key && $this->_allow === FALSE)
        {
            ...
            $this->response(
                ["response" => [
                    ...
                    "errors" => array(
                        array(
                            "code" => self::ERR_REST_INVALLID_API_KEY,
        ...
        if ($this->config->item('rest_enable_keys') && $use_key && empty($this->rest->key) === FALSE && $this->_check_access() === FALSE)
        {
            ...
            $this->response(
                ["response" => [
                    ...
                    "errors" => array(
                        array(
                            "code" => self::ERR_REST_UNAUTHORIZED_API_KEY,
                            "message" => $this->lang->line('text_rest_api_key_unauthorized')
                        )
                    )
        ...
        if (!method_exists($this, $controller_method))
        {
            $this->response(
                ["response" => [
                    ...
                    "errors" => array(
                        array(
                            "code" => self::HTTP_METHOD_NOT_ALLOWED,
                            "message" => $this->lang->line('text_rest_unknown_method')." -> ".$controller_method." -> ".json_encode($arguments)
                        )
                    )
                ]], self::HTTP_NOT_FOUND
            );
            ...
            if ($this->config->item('rest_enable_limits') && $this->_check_limit($controller_method) === FALSE)
            {
                $this->response(
                    ["response" => [
                        ...
                        "errors" => array(
                            array(
                                "code" => self::ERR_REST_API_KEY_TIME_LIMIT,
                                "message" => $this->lang->line('text_rest_api_key_time_limit')
                            )
                        )
                    ]], self::HTTP_UNAUTHORIZED
                );
            ...
            if($authorized === FALSE)
            {
                $this->response(
                    ["response" => [
                        ...
                        "errors" => array(
                            array(
                                "code" => self::ERR_REST_API_KEY_PERMISSIONS,
                                "message" => $this->lang->line('text_rest_api_key_permissions')
                            )
                        )
                    ]], self::HTTP_UNAUTHORIZED
                );
        ...
        try
        {
            // Check login required for method
            if(isset($this->arrAllowedMethods) && !empty($this->arrAllowedMethods) && array_key_exists($controller_method, $this->arrAllowedMethods))
            {
                // Load Lib_Rest_Auth_Tokens Library
                $this->load->library('v1/auth_tokens/Lib_Rest_Auth_Tokens');

                // Check token expired
                if(is_array($arrResult = $this->lib_rest_auth_tokens->checkAuthTokenExpired()))
                    return $this->response($arrResult['response'], $arrResult['HTTPCode']); // Set response
            }

            // Call method
            $arrResult = call_user_func_array([$this, $controller_method], $arguments);

            // Set response
            return $this->response($arrResult['response'], $arrResult['HTTPCode']);
        }
        catch (Exception $ex)
        {
                ...
                array(
                    'response' => array(
                        ...
                        'result' => array(
                            'errors' => array(
                                array(
                                    'code' => ($ex->getCode()>600) ? $ex->getCode() : self::ERR_REST_CI_EXCEPTION,
                                    'message' => $ex->getMessage()
                                )
                            )
                        )
                    )
                ), ($ex->getCode()>600) ? self::HTTP_UNAUTHORIZED : self::HTTP_INTERNAL_SERVER_ERROR
            );
    ...
    public function response($data = NULL, $http_code = NULL)
    {
        ...
        elseif ($data !== NULL)
        {
            ...
            if($this->config->item('rest_enable_profiling'))
            {
